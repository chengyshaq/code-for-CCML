...
function [ BestParameter, BestResult ] = adaptiveValidate( train_data, train_target, oldOptmParameter)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% tune the best parameters for CCML by crossvalidation
% Input
    ...
    for i=1:length(alpha_searchrange) % alpha
        for j=1:length(beta_searchrange) % beta
            for k = 1:length(gamma_searchrange) % gamma
                    fprintf('\n-   %d-th/%d: search parameter alpha and beta for CCML, alpha = %f, beta = %f, and gamma = %f',index, total, alpha_searchrange(i), beta_searchrange(j), gamma_searchrange(k));
                    index = index + 1;
                    ...
                    for cv = 1:num_cv
                        ...
                        [ccml_model] = CCML( cv_train_data, cv_train_target,optmParameter);
                        Outputs     = (cv_test_data*ccml_model.W)';
